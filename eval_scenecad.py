...
from util.postprocess_utils import postprocess_scenecad
...
def get_args_parser():
    ...
    parser.add_argument('--checkpoint', default="./checkpoints/pretrained_scenecad_ckpt.pth", type=str)
    return parser
...
def main(args):
    ...
    # Load test data
    dataset_eval = build_dataset('val', args)
...
def evaluate(model, data_loader, save_folder, args, save_primitive=True):
    model.eval()
    npy_folder = f"{save_folder}/npy_scenecad"
    vis_folder = f"{save_folder}/vis_scenecad"
    ...
    for batched_inputs in metric_logger.log_every(data_loader, 10, header):
        ...
        for b_i in range(bs):
            ...
            room_poly = postprocess_scenecad(save_polygons)
            for _ in range(room_poly.shape[0]):
                cv2.circle(img, room_poly[_].astype(np.uint8), 3, [255, 255, 255], -1)
                cv2.circle(img, room_poly[(_+1)%room_poly.shape[0]].astype(np.uint8), 3, [255, 255, 255], -1)
                cv2.line(img, room_poly[_].astype(np.uint8), room_poly[(_+1)%room_poly.shape[0]].astype(np.uint8), [255, 255, 255], 2)
            save_img_path = f'{vis_folder}/{img_name}.png'
            save_path = f'{npy_folder}/{img_name}.npy'
            print(f'generate: {img_name}')
            cv2.imwrite(save_img_path, img)
            np.save(save_path, room_poly)
