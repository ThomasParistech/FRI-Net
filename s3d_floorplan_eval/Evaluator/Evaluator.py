...
class Evaluator():
    ...
    def evaluate_scene(self, room_polys, room_types=None, window_door_lines=None, window_door_lines_types=None, show=False, name="ours", dataset_type="s3d"):
        ...
        # # visualize the predicted results and the gt results
        # import copy
        # pred_map = copy.deepcopy(input_img)
        # for room in room_polys:
        #     for _ in range(room.shape[0]-1):
        #         cv2.circle(pred_map, room[_].astype(np.uint8), 3, [255, 255, 255], -1)
        #         cv2.circle(pred_map, room[_+1].astype(np.uint8), 3, [255, 255, 255], -1)
        #         cv2.line(pred_map, room[_].astype(np.uint8), room[_+1].astype(np.uint8), [255, 255, 255], 2)
        # gt_map = copy.deepcopy(input_img)
        # for room in gt_polys_list:
        #     for _ in range(room.shape[0]-1):
        #         cv2.circle(gt_map, room[_].astype(np.uint8), 3, [255, 255, 255], -1)
        #         cv2.circle(gt_map, room[_+1].astype(np.uint8), 3, [255, 255, 255], -1)
        #         cv2.line(gt_map, room[_].astype(np.uint8), room[_+1].astype(np.uint8), [255, 255, 255], 2)
        # print(f'pred_len: {len(room_polys)}')
        # print(f'gt_len: {len(gt_polys_list)}')
        # cv2.imshow('pred', pred_map)
        # cv2.imshow('gt', gt_map)
        # cv2.waitKey(0)
    ...
    def get_quantitative(self, gt_polys, gt_polys_types, gt_window_doors, gt_window_doors_types, ignore_mask_region, pred_polys=None, pred_types=None, pred_window_doors=None, pred_window_doors_types=None, masks_list=None, img_size=(256, 256), dataset_type="s3d", img=None):
        def get_room_metric():
        ...
        def get_corner_metric():
            ...
            room_corners_metric = []
            import copy
            rec_map = copy.deepcopy(img)
            prec_map = copy.deepcopy(img)

            for pred_poly_ind, gt_poly_ind in enumerate(pred2gt_indices):
                p_poly = pred_polys[pred_poly_ind][:-1] # Last vertex = First vertex

                for idx, v in enumerate(p_poly):
                    cv2.circle(prec_map, p_poly[idx].astype(np.int32), 3, [255, 0, 0], -1)
                    cv2.circle(prec_map, p_poly[(idx+1)%len(p_poly)].astype(np.int32), 3, [255, 0, 0], -1)
                    cv2.line(prec_map, p_poly[idx].astype(np.int32), p_poly[(idx+1)%len(p_poly)].astype(np.int32), [255, 255, 255], 1)

                gt_poly = gt_polys[gt_poly_ind][:-1]

                for idx, v in enumerate(gt_poly):
                    cv2.circle(rec_map, gt_poly[idx].astype(np.int32), 3, [0, 0, 255], -1)
                    cv2.circle(rec_map, gt_poly[(idx+1)%len(gt_poly)].astype(np.int32), 3, [0, 0, 255], -1)
                    cv2.line(rec_map, gt_poly[idx].astype(np.int32), gt_poly[(idx+1)%len(gt_poly)].astype(np.int32), [255, 255, 255], 1)
                ...
                for v in gt_poly:
                    ...
                    if not p_poly_corner_metrics[v_min_dist_ind]:
                        ...
                        if v_tp:
                            cv2.circle(rec_map, v.astype(np.int32), 3, [0, 255, 0], -1)
                for idx in range(len(p_poly_corner_metrics)):
                    if p_poly_corner_metrics[idx]:
                        cv2.circle(prec_map, p_poly[idx].astype(np.int32), 3, [0, 255, 0], -1)

                room_corners_metric += p_poly_corner_metrics

            return room_corners_metric, prec_map, rec_map
        ...
        ###### metric for corner ######
        corner_metric, prec_map, rec_map = get_corner_metric()
